...
def get_arguments():
    ...
    parser.add_argument('--data-dir', type=str, default=SOURCE_DATA, help='Path to the directory containing the source dataset.')
    parser.add_argument('--source-train', type=str, default=TRAIN_SOURCE_LIST, help='Path to the file listing the images in the source dataset.')
    parser.add_argument('--source-test', type=str, default=TEST_SOURCE_LIST, help='Path to the file listing the images in the source dataset.')
    ...
    parser.add_argument('--is_adabn', type=bool, default=IS_ADABN, help='Whether to apply test mean and var rather than running.')
    parser.add_argument('--data-dir-target', type=str, default=TARGET_DATA, help='Path to the directory containing the target dataset.')
    parser.add_argument('--target-train', type=str, default=TRAIN_TARGET_LIST, help='Path to the file listing the images in the target dataset.')
    parser.add_argument('--target-test', type=str, default=TEST_TARGET_LIST, help='Path to the file listing the images in the target dataset.')
    ...
    parser.add_argument('--w-contrast', type=float, default=0.01, help='Weight of contrast loss.')
    parser.add_argument('--w-distill', type=float, default=0.1, help='Weight of distill loss.')
    ...
    parser.add_argument('--valid-steps', type=int, default=VALID_STEPS, help='Number of training steps.')
    parser.add_argument('--random-mirror', type=bool, default=RANDOM_MIRROR, help='Whether to randomly mirror the inputs during the training.')
    ...
    parser.add_argument('--save-result', type=bool, default=SAVE_RESULT, help='Whether to save the predictions.')
    parser.add_argument('--restore-from', type=str, default=RESTORE_FROM, help='Where to save snapshots of the model.')
    parser.add_argument('--gpu', type=str, default=GPU, help='choose gpu device.')
    parser.add_argument('--fold', type=str, default=FOLD, help='choose gpu device.')
    parser.add_argument('--power', type=float, default=POWER, help='Decay parameter to compute the learning rate.')
    parser.add_argument('--is-inverse', type=bool, default=IS_INVERSE, help='use inversed images.')
    parser.add_argument('--is-pseudo', type=bool, default=IS_PSEUDO, help='use pseudo labels.')
    parser.add_argument('--is-contrast', type=bool, default=IS_CONTRAST, help='apply contrast loss.')
    parser.add_argument('--target-mode', type=str, default=TARGET_MODE, help='choose gpu device.')
...
def main():
    ...
    contrast_dataset = Contrast_Polyp(root=args.data_dir_target, data_dir=args.target_train, mode='train', max_iter=None, is_mirror=args.random_mirror, is_pseudo=args.is_pseudo, is_inverse=args.is_inverse)
    test_target = Polyp(root=args.data_dir_target, data_dir=args.target_test, mode='test', is_mirror=False, is_inverse=args.is_inverse)
    contrast_loader_target = torch.utils.data.DataLoader(contrast_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
    test_loader_target = torch.utils.data.DataLoader(test_target, batch_size=1, shuffle=False, num_workers=args.num_workers)
    model = Deeplab(1, pretrained=True, contrast=args.is_contrast).cuda()
    optimizer = torch.optim.SGD(model.optim_parameters(args), lr=args.learning_rate, momentum=0.9, weight_decay=0.0005)
    ...
    for epoch in range(args.num_steps):
        ...
        for i_iter, batch in enumerate(contrast_loader_target):
            ...
            lr = adjust_learning_rate(optimizer=optimizer, i_iter=i_iter + epoch * contrast_dataset.__len__() / args.batch_size, length=contrast_dataset.__len__() / args.batch_size)
        batch_time = time.time() - tic
        print('Epoch: [{}/{}], Time: {:.2f}, lr: {:.6f}, Seg Loss: {:.6f}, Dis Loss: {:.6f}, Con Loss: {:.6f}'.format(epoch, args.num_steps, batch_time, lr, seg_loss, dis_loss, con_loss))
