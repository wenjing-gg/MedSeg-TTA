...
def test(model, dataloader, args, adabn_loader=False):
    metrics = Metrics(['recall', 'specificity', 'Dice', 'ACC_overall', 'IoU_poly', 'IoU_bg', 'IoU_mean'])
    if args.is_adabn:
        model.train()
    else:
        model.eval()
    with torch.no_grad():
        ...
        for i, data in bar:
            ...
            _recall, _specificity, _Dice, _ACC_overall, _IoU_poly, _IoU_bg, _IoU_mean = evaluate(output, gt)
            metrics.update(recall=_recall, specificity=_specificity, Dice=_Dice, ACC_overall=_ACC_overall, IoU_poly=_IoU_poly, IoU_bg=_IoU_bg, IoU_mean=_IoU_mean)
    metrics_result = metrics.mean(total_batch)
    print('Test Result:')
    print('recall: %.5f, specificity: %.5f, Dice: %.5f, ACC_overall: %.5f, IoU_poly: %.5f, IoU_bg: %.5f, IoU_mean: %.5f' % (metrics_result['recall'], metrics_result['specificity'], metrics_result['Dice'], metrics_result['ACC_overall'], metrics_result['IoU_poly'], metrics_result['IoU_bg'], metrics_result['IoU_mean']))
    print()
