...
from utils_brats import get_data_loader
...
def Savefeat(cfg, logger):
    ...
    def create_model():
        model = net_factory_3d(net_type="nnUNet", in_chns=1,
                            class_num=4)
        return model
    model = create_model()
    ...
    ncentroids = 30
    CAU_full = torch.load(CAU_full_path)
    CAU_full = CAU_full.reshape(ncentroids, 1, 512)
    ...
    with torch.no_grad():
        ...
        for idx, sampled_batch in enumerate(train_loader):
            ...
            batch, w, h,d = labels.size()
            newlabels = labels.reshape([batch, 1, w, h,d]).float()
            ...
            # print("ids = ",ids)
            single_image_objective_vectors = np.zeros([1, 512])
    ...
    os.makedirs('./selection_list', exist_ok=True)
    file = open(os.path.join('./selection_list', 'semiv1_512_SPH_al_APH_ist_%.2f_.txt' % per), 'w')     #@whq Selection list
...
class Class_Features:
    def __init__(self, numbers=19):
        ...
        self.tsne_data = 0
        self.pca_data = 0
        # self.class_features = np.zeros((19, 512))
    ...
    def calculate_mean_vector(self, feat_cls, outputs, labels_val, model):
        ...
        tensor1, *_ = torch.split(outputs_softmax, 1, dim=1)
        outputs_argmax=tensor1.float()
        outputs_pred = outputs_argmax
        scale_factor = F.adaptive_avg_pool3d(outputs_pred, 1)
        ...
        for n in range(feat_cls.size()[0]):
            for t in range(1):
                ...
                max_pool = nn.MaxPool3d(kernel_size=16)
                # 进行最大池化操作
                output = max_pool(s)
                # 展平
...
if __name__ == "__main__":
    ...
    _ARTIFACT_DIR = _SCRIPT_DIR.parent / "artifacts"
    parser.add_argument(
        "--config",
        nargs="?",
        type=str,
        default=str(_SCRIPT_DIR / "configs" / "SPH_to_SCH_source.yml"),
        help="Configuration file to use"
    )
    parser.add_argument("--source-root", type=str, default=str(_ARTIFACT_DIR / "source"))
    parser.add_argument("--target-root", type=str, default=str(_ARTIFACT_DIR / "target"))
    parser.add_argument("--mode", type=str, default="target_to_target")
    parser.add_argument("--img", type=str, default="t1c")
    parser.add_argument(
        "--logdir",
        type=str,
        default=str(_ARTIFACT_DIR / "model" / "nnUNet_t1c"),
    )
    parser.add_argument(
        "--model-path",
        type=str,
        default=str(_ARTIFACT_DIR / "model" / "nnUNet_t1c" / "best_model.pth"),
    )
    parser.add_argument(
        "--anchors-path",
        type=str,
        default=str(_ARTIFACT_DIR / "Reference_points_3dunet_t1c" / "SPH_cluster256_centroids_full_30.pkl"),
    )
    ...
    mode = args.mode
    img = args.img
    train_loader,valloader = get_data_loader(
    ...
    CAU_full_path = args.anchors_path
    Savefeat(cfg, logger)
