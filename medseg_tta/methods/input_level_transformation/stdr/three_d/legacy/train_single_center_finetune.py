...
parser.add_argument("--base_lr", type=float, default=0.01,  #0.03 for SGD   0.0001 for adam
...
_ARTIFACT_DIR = _LEGACY_DIR / "artifacts"
...
parser.add_argument(
    "--snapshot-root",
    type=str,
    default=str(_ARTIFACT_DIR / "model"),
)
...
def train(args, snapshot_path):
    ...
    for epoch_num in range(start_epoch,max_epoch):
        if train_flag :
            ...
            if epoch_num % 5 == 0:
                save_mode_path = os.path.join(
                    snapshot_path, "model_epoch" + str(epoch_num) + ".pth")
                util.save_checkpoint(
                    epoch_num, model, optimizer, loss, save_mode_path)
...
if __name__ == "__main__":
    ...
    snapshot_path = os.path.join(args.snapshot_root, "{}_{}".format(args.model, img))
