...
from metrics import cal_hd95,cal_dice,cal_RVE,IoU,PA
...
def train(config, train_loader, valid_loader, list_data):
    ...
    if resume1:
        upl_model.load_state_dict(torch.load('/root/autodl-tmp/ZDH/UPL-SFDA-main/save_model_brats/model-99.pth'))
        print('load model')
    ...
    def train_source(image, label):
        ...
        upl_model.aux_seg_1 = upl_model.aux_dec1(latent_A, blocks).squeeze(1)
        
        loss,dice1, dice2, dice3 = criterion(upl_model.aux_seg_1, upl_model.labA)
        loss.backward()
        optimizer.step()
    ...
    # 将模型切换到评估模式
        ...
        with torch.no_grad():
            ...
            #aux_seg_1 = upl_model.aux_dec1(latent_A, blocks).squeeze(1)
            aux_seg_1 = upl_model(image)
            # 计算损失
            ...
            hd95_wt, hd95_co, hd95_ec = cal_hd95(aux_seg_1, label)
            # cal_RVE,IoU,PA
            RVE_wt, RVE_co, RVE_ec = cal_RVE(aux_seg_1, label)
            iou_wt, iou_co, iou_ec = IoU(aux_seg_1, label)
            pa_wt, pa_co, pa_ec = PA(aux_seg_1, label, num_classes)

        
        # 返回损失和性能指标
        #return dice1.item(), dice2.item(), dice3.item(),hd95_wt, hd95_co, hd95_ec
        return dice1, dice2, dice3,hd95_wt, hd95_co, hd95_ec,RVE_wt, RVE_co, RVE_ec,iou_wt, iou_co, iou_ec,pa_wt, pa_co, pa_ec
    # 训练主循环
    train_flag = False
    for epoch in range(start_epoch,num_epochs):
        ...
        if train_flag:
            ...
            for i, (image, label, _, C) in enumerate(train_bar):
                # 调用训练源函数
                loss, dice1, dice2, dice3 = train_source(image, label)
                ...
                # 更新进度条
                train_bar.desc = f"Epoch [{epoch + 1}/{num_epochs}] dice:[{dice_mean_avg:.4f}] loss:[{loss:.4f}] Training"
            # 计算平均训练指标
        ...
        if (epoch + 1) % 10 == 0:
            ...
            with torch.no_grad():
                for image, label, _, _  in valid_bar:
                    ...
                    dice1,dice2,dice3,hd95_wt, hd95_co, hd95_ec,RVE_wt, RVE_co, RVE_ec,iou_wt, iou_co, iou_ec,pa_wt, pa_co, pa_ec = evaluate_source(image, label)
                ...
                with open(validation_results_file, 'a') as f:
                    ...
                    f.write(f"Val RVE: ET {avg_RVE_wt:.3f} TC {avg_RVE_co:.3f} WT {avg_RVE_ec:.3f}\n")
                    f.write(f"Val IoU: ET {avg_iou_wt:.3f} TC {avg_iou_co:.3f} WT {avg_iou_ec:.3f}\n")
                    f.write(f"Val PA: ET {avg_pa_wt:.3f} TC {avg_pa_co:.3f} WT {avg_pa_ec:.3f}\n")
                #train_flag = True
                # 保存最佳模型
                if train_flag ==False:
...
def main():
    ...
    parser.add_argument('--config', type=str, default="/root/autodl-tmp/ZDH/UPL-SFDA-main/config/train2d_source.cfg",
