...
class UNet(nn.Module):
    def __init__(
        ...
        if dataset == 'ssa' or dataset == 'brats':
            in_chns = params['network']['in_chns']
            n_classes = params['network']['n_classes_mms']
            ft_chns = params['network']['ft_chns_mms']
            self.pl_threshold = params['train']['pl_threshold_mms']
    ...
    def train_source(self, imagesa, labelsa):
        ...
        self.labA = labelsa
        # 只使用一个解码器进行前向传播
        x_shape = list(self.imgA.shape)
        if len(x_shape) == 5:
            [N, C, D, H, W] = x_shape
            new_shape = [N * D, C, H, W]
            self.imgA = torch.transpose(self.imgA, 1, 2)
            self.imgA = torch.reshape(self.imgA, new_shape)
        ...
        self.enc_opt.zero_grad()
        self.aux_dec1_opt.zero_grad()
        print(f"aux_seg_1:{self.aux_seg_1.shape}")
        print(f"labA:{self.labA.shape}")
        seg_loss_B = self.segloss(self.aux_seg_1, self.labA, one_hot=True)
        seg_loss_B.backward()
