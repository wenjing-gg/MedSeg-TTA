...
def get_data_loader(config,dataset,target):
    ...
    if dataset == 'mms':
        ...
        train_img = data_root_mms+'/train/img/{}'.format(target)
        train_lab = data_root_mms+'/train/lab/{}'.format(target)
        valid_img = data_root_mms+'/train/img/{}'.format(target)
        valid_lab = data_root_mms+'/train/lab/{}'.format(target)
        test_img = data_root_mms+'/train/img/{}'.format(target)
        test_lab = data_root_mms+'/train/lab/{}'.format(target)
    elif dataset == 'ssa':
        train_img = data_root_mms+'/train/img/{}'.format(target)
        train_lab = data_root_mms+'/train/lab/{}'.format(target)
        valid_img = data_root_mms+'/train/img/{}'.format(target)
...
def train(config,train_loader,valid_loader,test_loader,target,list_data):
    ...
    for epoch in range(num_epochs):
        train_bar = tqdm(itertools.islice(train_loader, 3), desc=f"Epoch [{epoch + 1}/{num_epochs}] Training", mininterval=0.1)
        ...
        if (epoch+1) % valid_epochs == 0:
            ...
            with torch.no_grad():
                upl_model.eval()
                for it,(xt,xt_label,xt_name,lab_Imag) in enumerate(itertools.islice(train_loader, 5)):  
                    xt = xt.to(device)
            ...
            avg_dice = current_dice / (it + 1)  # Ensure 'it' is correctly used here
