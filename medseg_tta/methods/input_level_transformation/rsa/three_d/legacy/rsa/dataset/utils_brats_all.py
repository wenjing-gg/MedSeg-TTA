...
def get_brats2021_base_transform_all():
    base_transform = [
        ...
        # transforms.Resized(keys=['label'], spatial_size=(128,128,128),mode='nearest'),
        # transforms.Resized(keys=['image'], spatial_size=(128,128,128),mode='trilinear'),
        transforms.Resized(keys=['label'], spatial_size=(64,64,64),mode='nearest'),
        transforms.Resized(keys=['image'], spatial_size=(64,64,64),mode='trilinear'),
...
class niiDataset(Dataset):
    def __init__(self, sample_dir, image_path, img, phase='test'):
        
        self.img_path = image_path
        self.img = img
        self.phase  =phase
        self.case_names = sorted(os.listdir(image_path))
        self.sample_files = list_files(sample_dir) if sample_dir else []
...
def get_data_loader(sample_dir=None,source_root=None,target_root=None,train_path='train',test_path='test',batch_train=1,batch_test=1,nw = 4,img='t2f',mode='source_to_source'):
    ...
    train_dataset = niiDataset(sample_dir,train_img,img=img, phase='train')
    test_dataset = niiDataset(sample_dir,test_img,img=img,phase='test')
