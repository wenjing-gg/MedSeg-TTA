...
def train_on_target(args, device, epoch):
    ...
    # ---------------- Paths --------------------------------------------------
    result_dir = os.path.join("DLTTA_2D", "tta2d_results")
    weights_dir = os.path.join(result_dir, "weights")
    os.makedirs(weights_dir, exist_ok=True)
    ...
    optimizer = torch.optim.SGD(params, lr=0.0000001)
    ...
    for i in range(epoch):
        ...
        for step, batch in enumerate(train_bar):
            ...
            loss_dc =entropy_loss(pred_seg, c=2)   
        ...
        if (i+1) % 4 == 0:  # 每5个epoch保存一次
            # torch.save(model.state_dict(), os.path.join(weights_dir, f"{model_tag}_tta2d_{args.dataset}_epoch{i}_{timestamp}.pth"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_dir", type=str, default="./checkpoints-DLTTA", help="保存 / 查找权重的目录")
    parser.add_argument("--model_path", type=str, default=f"/home/yuwenjing/DeepLearning_ywj/tta/tent/checkpoint_{dataset_name}/unet2d_best_{dataset_name}.pth",
