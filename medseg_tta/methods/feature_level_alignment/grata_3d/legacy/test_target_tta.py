...
def test_on_target(args, device):
    ...
    print(f'{'=' * 40}\n')
    try:
        result_dir = '/home/yuwenjing/DeepLearning_ywj/tta/GraTa-3d/checkpoints/tta_results'
        ...
        grata_config = get_default_grata_config()
        grata_config.lr = args.lr
        grata_config.aux_loss = getattr(args, 'aux_loss', 'ent')
        grata_config.pse_loss = getattr(args, 'pse_loss', 'consis')
        grata_config.optimizer = getattr(args, 'optimizer', 'Adam')
        grata_model = create_grata_model(model, grata_config, device)
        for param in grata_model.get_model().parameters():
            if param.device != device:
                param.data = param.data.to(device)
        print(f'🔍 模型参数设备检查: {next(model.parameters()).device}')
        print(f'🔍 GraTa模型参数设备检查: {next(grata_model.get_model().parameters()).device}')
        ...
        for batch in tqdm(target_test_loader, desc='GraTa推理进度'):
            if len(batch) == 3:
            ...
            outputs = grata_model.adapt_and_predict(imgs)
        ...
        result_file = os.path.join(result_dir, f'GraTa_{args.img}_{timestamp}.txt')
        with open(result_file, 'w') as f:
            f.write(report)
        metric_df = pd.DataFrame(metric_dict)
        csv_file = os.path.join(result_dir, f'GraTa_{args.img}_{timestamp}.csv')
        metric_df.to_csv(csv_file, mode='w', header=True, index=False)
        summary_df = pd.DataFrame(summary_stats)
        summary_csv = os.path.join(result_dir, f'GraTa_{args.img}_{timestamp}_summary.csv')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='GraTa算法在BraTS数据集上的测试时适应')
    ...
    parser.add_argument('--checkpoint_dir', type=str, default='/home/yuwenjing/DeepLearning_ywj/tta/GraTa-3d/checkpoints', help='包含预训练权重的检查点目录')
    ...
    parser.add_argument('--lr', type=float, default=0.0001, help='GraTa学习率')
    ...
    parser.add_argument('--aux_loss', type=str, default='ent', choices=['ent', 'consis'], help='GraTa辅助损失类型')
    parser.add_argument('--pse_loss', type=str, default='consis', choices=['ent', 'consis'], help='GraTa伪损失类型')
    parser.add_argument('--optimizer', type=str, default='Adam', choices=['Adam', 'SGD'], help='GraTa基础优化器')
