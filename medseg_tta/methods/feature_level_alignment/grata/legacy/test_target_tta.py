...
def test_on_target(args, device):
    ...
    print(f'{'=' * 40}\n')
    try:
        result_dir = '/home/yuwenjing/DeepLearning_ywj/tta/*/checkpoints/tta_results'
        ...
        result_file = os.path.join(result_dir, f'*_{args.img}_{timestamp}.txt')
        with open(result_file, 'w') as f:
            f.write(report)
        metric_df = pd.DataFrame(metric_dict)
        csv_file = os.path.join(result_dir, f'*_{args.img}_{timestamp}.csv')
        metric_df.to_csv(csv_file, mode='w', header=True, index=False)
        summary_df = pd.DataFrame(summary_stats)
        summary_csv = os.path.join(result_dir, f'*_{args.img}_{timestamp}_summary.csv')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='* 算法在BraTS数据集上的测试')
    ...
    parser.add_argument('--checkpoint_dir', type=str, default='/home/yuwenjing/DeepLearning_ywj/tta/*/checkpoints', help='包含预训练权重的检查点目录')
    ...
    print(summary)
    result_dir = '/home/yuwenjing/DeepLearning_ywj/tta/*/checkpoints/tta_results'
    summary_file = os.path.join(result_dir, f'*_summary_{args.model_type}_{start_time.strftime('%Y%m%d_%H%M%S')}.txt')
