...
def block_ind(mask, sz_block=64, sz_pad=0):
    ...
    参数:
    - shape: 输入数组的形状，格式为 (x, y, z)。
    - sz_block: 每个块的大小，默认为 64。
    - sz_pad: 在每个维度上额外添加的块数，默认为 0。
    
    返回:
    - ind_block: 一个二维数组，每一行表示一个块的索引范围 [xmin, xmax, ymin, ymax, zmin, zmax]。
    """
    x, y, z = mask.shape
    
    # 计算每个维度上的块数
    nx = int(np.ceil(x / sz_block)) + sz_pad
    ny = int(np.ceil(y / sz_block)) + sz_pad
    nz = int(np.ceil(z / sz_block)) + sz_pad
    ...
    for ii in range(nx):
        for jj in range(ny):
            for kk in range(nz):
                xmin = ii * sz_block
                xmax = min(xmin + sz_block - 1, x - 1)
                ymin = jj * sz_block
                ymax = min(ymin + sz_block - 1, y - 1)
                zmin = kk * sz_block
                zmax = min(zmin + sz_block - 1, z - 1)
                
                # 确保块的大小始终为 sz_block
                if xmax - xmin + 1 < sz_block:
                    xmin = max(0, xmax - sz_block + 1)
                if ymax - ymin + 1 < sz_block:
                    ymin = max(0, ymax - sz_block + 1)
                if zmax - zmin + 1 < sz_block:
                    zmin = max(0, zmax - sz_block + 1)
                
                ind_block.append([xmin, xmax, ymin, ymax, zmin, zmax])
    
    return np.array(ind_block)
    """
def block_ind(mask, sz_block=64, sz_pad=0):
...
def block2brain(pass_ii,blocks, inds, mask):
    ...
    count_pass = 0
    for tt in np.arange(blocks.shape[0]):
        if tt in pass_ii:
            count_pass += 1
        inds_this = inds[tt + count_pass, :]
        #print("11",pass_ii,count_pass,inds.shape,inds_this,inds_this.shape,vol_brain.shape,blocks.shape,tt)
