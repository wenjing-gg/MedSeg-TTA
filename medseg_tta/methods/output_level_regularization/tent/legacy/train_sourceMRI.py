    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            for imgs, labels, *_ in test_pbar:
                ...
                dice_list[0].append(dice1.item() if torch.is_tensor(dice1) else dice1)
                dice_list[1].append(dice2.item() if torch.is_tensor(dice2) else dice2)
                dice_list[2].append(dice3.item() if torch.is_tensor(dice3) else dice3)
                hd95_list[0].append(hd95_ET.item() if torch.is_tensor(hd95_ET) else hd95_ET)
                hd95_list[1].append(hd95_TC.item() if torch.is_tensor(hd95_TC) else hd95_TC)
                hd95_list[2].append(hd95_WT.item() if torch.is_tensor(hd95_WT) else hd95_WT)
                IoU_list[0].append(IoU_ET.item() if torch.is_tensor(IoU_ET) else IoU_ET)
                IoU_list[1].append(IoU_TC.item() if torch.is_tensor(IoU_TC) else IoU_TC)
                IoU_list[2].append(IoU_WT.item() if torch.is_tensor(IoU_WT) else IoU_WT)
                RVE_list[0].append(RVE_ET.item() if torch.is_tensor(RVE_ET) else RVE_ET)
                RVE_list[1].append(RVE_TC.item() if torch.is_tensor(RVE_TC) else RVE_TC)
                RVE_list[2].append(RVE_WT.item() if torch.is_tensor(RVE_WT) else RVE_WT)
                sensitivity_list[0].append(sensitivity_ET.item() if torch.is_tensor(sensitivity_ET) else sensitivity_ET)
                sensitivity_list[1].append(sensitivity_TC.item() if torch.is_tensor(sensitivity_TC) else sensitivity_TC)
                sensitivity_list[2].append(sensitivity_WT.item() if torch.is_tensor(sensitivity_WT) else sensitivity_WT)
                ppv_list[0].append(ppv_ET.item() if torch.is_tensor(ppv_ET) else ppv_ET)
                ppv_list[1].append(ppv_TC.item() if torch.is_tensor(ppv_TC) else ppv_TC)
                ppv_list[2].append(ppv_WT.item() if torch.is_tensor(ppv_WT) else ppv_WT)
                metric_count += 1
                test_pbar.set_postfix({'val_loss': f'{test_loss / metric_count:.4f}', 'dice': f'{dice1.item():.2f}/{dice2.item():.2f}/{dice3.item():.2f}'})
        avg_test_loss = test_loss / len(test_loader)
        avg_dice = [np.mean(dice_list[i]) for i in range(3)]
        std_dice = [np.std(dice_list[i]) for i in range(3)]
        avg_hd95 = [np.mean(hd95_list[i]) for i in range(3)]
        std_hd95 = [np.std(hd95_list[i]) for i in range(3)]
        avg_IoU = [np.mean(IoU_list[i]) for i in range(3)]
        std_IoU = [np.std(IoU_list[i]) for i in range(3)]
        avg_RVE = [np.mean(RVE_list[i]) for i in range(3)]
        std_RVE = [np.std(RVE_list[i]) for i in range(3)]
        avg_sensitivity = [np.mean(sensitivity_list[i]) for i in range(3)]
        std_sensitivity = [np.std(sensitivity_list[i]) for i in range(3)]
        avg_ppv = [np.mean(ppv_list[i]) for i in range(3)]
        std_ppv = [np.std(ppv_list[i]) for i in range(3)]
        ...
        if current_avg_dice > best_avg_dice or (current_avg_dice == best_avg_dice and current_avg_hd95 < best_avg_hd95):
            ...
            best_epoch = epoch
            best_model_path = os.path.join(checkpoint_dir, 'unet3d_best.pth')
            ...
            log_content += f'Dice (ET/TC/WT): {avg_dice[0]:.4f}±{std_dice[0]:.4f}/{avg_dice[1]:.4f}±{std_dice[1]:.4f}/{avg_dice[2]:.4f}±{std_dice[2]:.4f}\n'
            log_content += f'HD95 (ET/TC/WT): {avg_hd95[0]:.4f}±{std_hd95[0]:.4f}/{avg_hd95[1]:.4f}±{std_hd95[1]:.4f}/{avg_hd95[2]:.4f}±{std_hd95[2]:.4f}\n'
            log_content += f'IoU (ET/TC/WT): {avg_IoU[0]:.4f}±{std_IoU[0]:.4f}/{avg_IoU[1]:.4f}±{std_IoU[1]:.4f}/{avg_IoU[2]:.4f}±{std_IoU[2]:.4f}\n'
            log_content += f'RVE (ET/TC/WT): {avg_RVE[0]:.4f}±{std_RVE[0]:.4f}/{avg_RVE[1]:.4f}±{std_RVE[1]:.4f}/{avg_RVE[2]:.4f}±{std_RVE[2]:.4f}\n'
            log_content += f'Sensitivity (ET/TC/WT): {avg_sensitivity[0]:.4f}±{std_sensitivity[0]:.4f}/{avg_sensitivity[1]:.4f}±{std_sensitivity[1]:.4f}/{avg_sensitivity[2]:.4f}±{std_sensitivity[2]:.4f}\n'
            log_content += f'PPV (ET/TC/WT): {avg_ppv[0]:.4f}±{std_ppv[0]:.4f}/{avg_ppv[1]:.4f}±{std_ppv[1]:.4f}/{avg_ppv[2]:.4f}±{std_ppv[2]:.4f}\n'
            log_content += '-' * 40 + '\n'
...
def test_on_target(args):
    ...
    print(f'使用设备：{device}')
    model = UNet3d(in_chns=4, n_classes=4).to(device)
    best_model_path = os.path.join(args.checkpoint_dir, 'unet3d_best.pth')
