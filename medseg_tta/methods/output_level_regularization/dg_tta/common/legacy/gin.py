...
class GINGroupConv(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.layers.append(GradlessGCReplayNonlinBlock(out_channel=interm_channel, in_channel=in_channel, scale_pool=self.scale_pool, layer_id=0))
        for ii in range(self.n_layer - 2):
            self.layers.append(GradlessGCReplayNonlinBlock(out_channel=interm_channel, in_channel=interm_channel, scale_pool=self.scale_pool, layer_id=ii + 1))
        self.layers.append(GradlessGCReplayNonlinBlock(out_channel=self.out_channel, in_channel=interm_channel, scale_pool=self.scale_pool, layer_id=self.n_layer - 1, use_act=False))
    ...
    def forward(self, x_in):
        ...
        if self.out_norm == 'frob':
            ...
            if mode == '2d':
                ...
                raise ValueError()
            mixed = mixed * (1.0 / (_self_frob + 1e-05)) * _in_frob
        return mixed

def gin_aug(input):
    cfg = dict(IN_CHANNELS=1, N_LAYER=4, INTERM_CHANNELS=2)
    gin_group_conv = GINGroupConv(cfg)
    input = gin_group_conv(input)
    return input

def gin_hook(module, input):
    get_internal_augmentation_enabled = True
