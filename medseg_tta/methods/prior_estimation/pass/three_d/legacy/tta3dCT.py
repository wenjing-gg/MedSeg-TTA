    ...
    try:
        ...
        def line_pair(metric_name_cn, key, higher_is_better=True, nd=4, nd_std=4):
            ...
            return (f"{metric_name_cn:<12} "
                    f"pass3D: {b_mean:.{nd}f} ± {b_std:.{nd_std}f} | "
...
def train_on_target(args, device, epoch):
    ...
    # ---------------- Paths --------------------------------------------------
    result_dir = os.path.join("pass_3D", "tta3d_results")
    weights_dir = os.path.join(result_dir, "weights")
    os.makedirs(weights_dir, exist_ok=True)
    ...
    for i in range(epoch):
        ...
        for step, batch in enumerate(train_bar):
            ...
            # target_domain_label = bin_outputs  
            # m_p = m(bin_outputs)
            # 如果有非1的数，就变0
...
def main():
    ...
    parser.add_argument('--lr',           type=float, default=1e-5, help='TENT 学习率（稳健推荐：1e-5）')
