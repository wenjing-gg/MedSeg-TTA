...
from base_tta import BaseAdapter
...
from utils_unet import parse_config, set_random
####
...
class Offline_Adapter(BaseAdapter):
    ...
    def init_from_source_model(self):
        print('init from source model')
        args.pretrained_model = r"UNET3D-ORI.pth"
        ...
        elif  args.arch == 'unet_3d':
                print('load unet3d')
                parser1 = argparse.ArgumentParser(description='config file')
                default_config = os.path.normpath(
                    os.path.join(os.path.dirname(__file__), '..', '..', 'common', 'legacy', 'config', 'train3d.cfg')
                )
                parser1.add_argument('--config', type=str, default=default_config,
                                    help='Path to the configuration file')
                args1 = parser1.parse_args()
                config1 = args1.config
                config1 = parse_config(config1)
                print(config1)
                # args = {**vars(args)}
                # base_model = UNet3d(config1)
                source_model = r'UNET3D-ORI.pth'
                self.model = UNet3d(config1)
                self.model.load_state_dict(torch.load(source_model))
    ...
    def evaluate(self):
        ...
        for epoch in range(start_epoch, self.args.num_epochs):
            ...
            if  (epoch + 1) % 1 == 0:
                ...
                torch.save(saved_model, join(self.model_folder,f'model_{epoch}.model'))
                self.val(epoch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default="Tent-TTA", required=False,choices=['Moment-TTA', 'Tent-TTA', 'RN-CR-TTA'],
                        help='Model name.')
    parser.add_argument('--arch', default="unet_3d", required=False,
                        help='Network architecture.')
    parser.add_argument('--num_classes', default=4, required=False,
                        help='Num of classes.')
    ...
    parser.add_argument('--tag', default="Base2_test", required=False,
                        help='Run identifier.')
    parser.add_argument('--patch_size', type=int, nargs='+', default=[128, 128], required=False,
                        help='patch size.')
    parser.add_argument('--batch_size', type=int, default=1, required=False,
                        help='batch size.')
    parser.add_argument('--initial_lr', type=float, default=1e-5, required=False,
    ...
    parser.add_argument('--num_epochs', type=int, default=100, required=False,
                        help='num_epochs.')
    parser.add_argument('--pretrained_model', default=r'UNET3D-ORI.pth', required=False,
