...
class Weighted_self_entropy_loss():
    ...
    def self_entropy_loss(self, probs):
        log_p: Tensor = (probs[:, self.idc, ...] + 1e-10).log()
        mask: Tensor = probs[:, self.idc, ...].type((torch.float32))
        mask_weighted = torch.einsum("bcdwh,c->bcdwh", [mask, Tensor(self.weights).to(mask.device)])
        loss = - torch.einsum("bcdwh,bcdwh->", [mask_weighted, log_p])

        # mask_weighted = torch.einsum("bcdwh,c->bcdwh", [mask, Tensor(self.weights).to(mask.device).unsqueeze(-1).unsqueeze(-1)])
        loss /= mask.sum() + 1e-10
        return loss
...
class KL_class_ratio_entropy_loss():
    ...
    def __call__(self, pred: Tensor) -> Tensor:
        ...
        for i, c in enumerate(self.idc):
            # 对最后三个维度求和（深度、高度、宽度）
            class_sum = pred[:, c].sum([-3, -2, -1])  # 对 128x128x128 的三个维度求和
            batch_ratio[:, i] = class_sum / (h * w * d)  # h, w, d 分别是 128, 128, 128
...
class Constrain_prior_w_self_entropy_loss():
    ...
    def self_entropy_loss(self, probs):
        ...
        log_p: Tensor = (probs[:, self.idc, ...] + 1e-10).log()
        mask: Tensor = probs[:, self.idc, ...].type((torch.float32))
        mask_weighted = torch.einsum("bcdwh,c->bcdwh", [mask, Tensor(self.weights_se).to(mask.device)])
        loss = - torch.einsum("bcdwh,bcdwh->", [mask_weighted, log_p])
        loss /= mask.sum() + 1e-10
        return loss
