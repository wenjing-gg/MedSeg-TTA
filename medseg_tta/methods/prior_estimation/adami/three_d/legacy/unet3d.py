...
class UNet3d(nn.Module):
    ...
    def forward(self, x,feat = False):
        ...
        if feat:
            return self.aux_seg_1,latent_A
        return self.aux_seg_1
    def forward_with_features(self, x):
        """
        返回分割输出 + 多层特征 (浅层/中层/深层)
        """
        blocks, latent = self.enc(x)
        out = self.aux_dec1(latent, blocks).softmax(1)

        features = {
            "shallow": blocks[0],  # (16, D, H, W)
            "middle": blocks[2],   # (64, D/4, H/4, W/4)
            "deep": latent         # (256, D/16, H/16, W/16)
        }
        return out, features
