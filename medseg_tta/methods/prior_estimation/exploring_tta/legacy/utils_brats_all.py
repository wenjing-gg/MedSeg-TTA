...
def resize_and_normalize(image, label, target_shape, num_class):
    ...
    resized_label = resized_label.squeeze(0)
...
class UnpairedDataset(Dataset):
    ...
    def __init__(self, A_path, B_path):
        listA = [k for k in os.listdir(A_path) if not k.startswith('.')]
        listB = [k for k in os.listdir(B_path) if not k.startswith('.')]
        ...
        print(f'数据集统计 - A路径有效文件数: {self.Asize}, B路径有效文件数: {self.Bsize}')
...
def get_brats2021_base_transform():
    base_transform = [transforms.SpatialPadD(keys=['image', 'label'], spatial_size=(218, 218, 218), method='symmetric', mode='constant'), transforms.Resized(keys=['label'], spatial_size=(128, 128, 128), mode='nearest'), transforms.Resized(keys=['image'], spatial_size=(128, 128, 128), mode='trilinear'), transforms.NormalizeIntensityd(keys=['image'], nonzero=True, dtype=np.float32)]
    return base_transform

def get_brats2021_base_transform_all():
    base_transform = [transforms.ConcatItemsd(keys=['t1c', 't1n', 't2w', 't2f'], name='image', dim=0), transforms.DeleteItemsd(keys=['t1c', 't1n', 't2w', 't2f']), transforms.SpatialPadD(keys=['image', 'label'], spatial_size=(218, 218, 218), method='symmetric', mode='constant'), transforms.Resized(keys=['label'], spatial_size=(128, 128, 128), mode='nearest'), transforms.Resized(keys=['image'], spatial_size=(128, 128, 128), mode='trilinear'), transforms.NormalizeIntensityd(keys=['image'], nonzero=True, dtype=np.float32)]
    return base_transform

def get_brats2021_train_target_transform():
    base_transform = get_brats2021_base_transform()
    data_aug = [transforms.RandGaussianSmoothd(keys='image', prob=0.15, sigma_x=(0.5, 1.5), sigma_y=(0.5, 1.5), sigma_z=(0.5, 1.5)), transforms.RandAdjustContrastd(keys='image', prob=0.15, gamma=(0.7, 1.3)), transforms.EnsureTyped(keys=['image', 'label'])]
...
class niiDataset(Dataset):
    ...
    def __init__(self, image_path, img, phase='test'):
        ...
        self.img = img
        self.phase = phase
        self.case_names = [k for k in os.listdir(image_path) if not k.startswith('.')]
        print(f'成功加载{len(self.case_names)}个有效病例，已自动跳过隐藏文件')
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            print(f'加载文件 {name} 时出错: {e}')
            if index > 0:
                return self.__getitem__(0)
            else:
                raise RuntimeError(f'无法加载任何样本: {e}')
...
def get_data_loader(source_root, target_root, batch_train, batch_test, nw=4, img='t2f', mode='source_to_source'):

    def validate_path(path, desc):
        if not os.path.exists(path):
            raise FileNotFoundError(f'{desc}路径不存在: {path}')
        print(f'已验证{desc}路径: {path}')
    if mode == 'source_to_source':
        train_img = os.path.join(source_root, 'train')
        test_img = os.path.join(source_root, 'test')
        validate_path(train_img, '源域训练集')
        validate_path(test_img, '源域测试集')
    elif mode == 'source_to_target':
        train_img = os.path.join(source_root, 'train')
        test_img = os.path.join(target_root, '')
        validate_path(train_img, '源域训练集')
        validate_path(test_img, '目标域测试集')
    elif mode == 'target_to_target':
        train_img = os.path.join(target_root, '')
        test_img = os.path.join(target_root, '')
        validate_path(train_img, '目标域训练/测试集')
    else:
    ...
    try:
        ...
        train_loader = DataLoader(train_dataset, batch_size=batch_train, num_workers=nw, shuffle=True, drop_last=False, pin_memory=True)
        test_loader = DataLoader(test_dataset, batch_size=batch_test, num_workers=nw, shuffle=False, drop_last=False, pin_memory=True)
        print('数据加载完成')
        print('训练数据集大小:' + str(len(train_dataset)))
        print('测试数据集大小:' + str(len(test_dataset)))
...
if __name__ == '__main__':
    ...
    img = 'all'
    train_loader, val_loader = get_data_loader(source_root=source_root, target_root=target_root, train_path='train', test_path='test', batch_train=batch_train, batch_test=batch_test, nw=num_workers, img=img, mode=mode)
    print('数据加载完成')
