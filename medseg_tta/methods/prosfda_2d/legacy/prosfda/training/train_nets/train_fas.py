...
def train(args):
    ...
    tr_dataloader = torch.utils.data.DataLoader(tr_dataset, batch_size=batch_size, num_workers=num_threads, shuffle=shuffle, pin_memory=True, collate_fn=source_collate_fn_tr_fda)
    ts_dataloader = torch.utils.data.DataLoader(ts_dataset, batch_size=batch_size, num_workers=num_threads // 2, shuffle=False, pin_memory=True, collate_fn=collate_fn_ts)
    model = UNet_FAS()
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for iter, batch in enumerate(tr_dataloader):
            ...
            with autocast():
                ...
                compact_loss = F.l1_loss(global_f_fda_data, global_f_data.detach(), reduction='mean') + F.l1_loss(global_f_data, global_f_fda_data.detach(), reduction='mean')
                seg_loss = criterion(output_data[:, 0], (seg[:, 0] > 0) * 1.0) + criterion(output_data[:, 1], (seg[:, 0] == 2) * 1.0) + criterion(output_fda_data[:, 0], (seg[:, 0] > 0) * 1.0) + criterion(output_fda_data[:, 1], (seg[:, 0] == 2) * 1.0)
        ...
        if epoch % save_interval == 0:
            saved_model = {'epoch': epoch + 1, 'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict()}
