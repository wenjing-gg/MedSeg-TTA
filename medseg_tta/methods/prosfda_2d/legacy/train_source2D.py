...
def calculate_dice_score(pred: torch.Tensor, target: torch.Tensor, smooth: float=1e-06) -> float:
...
def calculate_iou(pred: torch.Tensor, target: torch.Tensor, smooth: float=1e-06) -> float:
    pred_binary = (pred > 0.5).float()
    target_binary = (target > 0.5).float()
    batch_size = pred.shape[0]
    iou_scores = []
...
def calculate_sensitivity(pred: torch.Tensor, target: torch.Tensor, threshold: float=0.5, eps: float=1e-06) -> float:
...
def calculate_ppv(pred: torch.Tensor, target: torch.Tensor, smooth: float=1e-06) -> float:
    pred_binary = (pred > 0.5).float()
    target_binary = (target > 0.5).float()
    batch_size = pred.shape[0]
    ppv_scores = []
...
def train_epoch(model: nn.Module, train_loader: torch.utils.data.DataLoader, criterion: nn.Module, optimizer: optim.Optimizer, scheduler: optim.lr_scheduler._LRScheduler, device: torch.device, epoch: int, total_epochs: int) -> tuple:
    model.train()
    epoch_loss = 0.0
...
def validate_epoch(model: nn.Module, val_loader: torch.utils.data.DataLoader, criterion: nn.Module, device: torch.device, epoch: int, total_epochs: int) -> tuple:
    model.eval()
    val_loss = 0.0
    total_metrics = {'dice': 0.0, 'iou': 0.0, 'sensitivity': 0.0, 'ppv': 0.0, 'hd95': 0.0}
...
def test_model(model: nn.Module, test_loader: torch.utils.data.DataLoader, criterion: nn.Module, device: torch.device, dataset_type: str) -> dict:
    model.eval()
    test_loss = 0.0
    total_metrics = {'dice': 0.0, 'iou': 0.0, 'sensitivity': 0.0, 'ppv': 0.0, 'hd95': 0.0}
...
def train(args):
    ...
    if hasattr(args, 'dataset_type') and args.dataset_type:
        train_loader, test_loader, dataset_type = get_data_loaders_2d(dataset_type=args.dataset_type, subfolder=getattr(args, 'subfolder', None), base_dir=getattr(args, 'base_dir', '/home/yuwenjing/data/tta_dataset'), batch_size_train=args.batch_train, batch_size_val=getattr(args, 'batch_test', args.batch_train), num_workers=args.num_workers, train_split=args.train_split, image_size=(args.image_size, args.image_size))
    else:
        train_loader, test_loader, dataset_type = get_data_loaders_2d(image_dir=args.image_dir, mask_dir=args.mask_dir, batch_size_train=args.batch_train, batch_size_val=getattr(args, 'batch_test', args.batch_train), num_workers=args.num_workers, train_split=args.train_split, image_size=(args.image_size, args.image_size))
    ...
    criterion = SegmentationLoss2D(ce_weight=1.0, dice_weight=1.0, class_weights=class_weights, smooth=1e-06, ignore_background=False)
    ...
    for epoch in range(args.epochs):
        if patience_counter >= args.patience:
            ...
            break
        train_loss, train_metrics = train_epoch(model, train_loader, criterion, optimizer, scheduler, device, epoch, args.epochs)
        test_loss, test_metrics = validate_epoch(model, test_loader, criterion, device, epoch, args.epochs)
        print(f'Epoch [{epoch + 1}/{args.epochs}] Results:')
        print(f'  Train - Loss: {train_loss:.4f}, Dice: {train_metrics['dice']:.4f}')
        print(f'  Test  - Loss: {test_loss:.4f}, Dice: {test_metrics['dice']:.4f}, IoU: {test_metrics['iou']:.4f}, Sens: {test_metrics['sensitivity']:.4f}, PPV: {test_metrics['ppv']:.4f}, HD95: {test_metrics['hd95']:.2f}')
    ...
    if os.path.exists(best_model_path):
        ...
        final_test_metrics = test_model(model, test_loader, criterion, device, dataset_type)
        final_log_content = f'\n[{datetime.datetime.now():%Y-%m-%d %H:%M:%S}]\nFINAL TEST RESULTS - {dataset_type}\nBest Epoch: {best_epoch}\nFinal Test Dice: {final_test_metrics['dice']:.4f}\nFinal Test IoU: {final_test_metrics['iou']:.4f}\nFinal Test Sensitivity: {final_test_metrics['sensitivity']:.4f}\nFinal Test PPV: {final_test_metrics['ppv']:.4f}\nFinal Test HD95: {final_test_metrics['hd95']:.2f}\n{'=' * 60}\n'
        log_file = os.path.join(args.checkpoint_dir, f'training_log_{dataset_type}.txt')
        with open(log_file, 'a') as f:
