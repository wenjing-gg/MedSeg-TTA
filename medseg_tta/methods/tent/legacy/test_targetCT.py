...
class CTDataset3D(Dataset):
    ...
    def __init__(self, image_dir: str, mask_dir: str, phase: str='test', image_size: Tuple[int, int, int]=(128, 128, 128), spacing: Tuple[float, float, float]=(1.0, 1.0, 1.0), intensity_range: Tuple[float, float]=(-200, 400), normalize: bool=True):
...
def get_ct_test_loader(image_dir: str=None, mask_dir: str=None, dataset_type: str=None, subfolder: str=None, base_dir: str='/home/yuwenjing/data/tta_dataset', target_dir: str=None, batch_size: int=2, num_workers: int=4, image_size: Tuple[int, int, int]=(128, 128, 128), spacing: Tuple[float, float, float]=(1.0, 1.0, 1.0), intensity_range: Tuple[float, float]=(-200, 400)) -> Tuple[DataLoader, str]:
    ...
    test_dataset = CTDataset3D(image_dir=image_dir, mask_dir=mask_dir, phase='test', image_size=image_size, spacing=spacing, intensity_range=intensity_range, normalize=True)
...
def test_on_target(args, device):
    ...
    try:
        ...
        target_test_loader, _ = get_ct_test_loader(image_dir=image_dir, mask_dir=mask_dir, target_dir=None, dataset_type=None, batch_size=args.batch_size, num_workers=args.num_workers, image_size=(args.image_size, args.image_size, args.image_size), spacing=args.spacing, intensity_range=args.intensity_range)
        with torch.no_grad():
        ...
        lines = ['=' * 40, f'测试时间: {timestamp}', '测试配置:', f'- 图像模态: {args.img}', f'- 模型类型: {model_name}', f'- 模型路径: {best_model_path}', f'- 测试数据: {args.target_dir}', f'- 图像目录: {image_dir}', f'- 掩码目录: {mask_dir}', '', '性能指标(均值 ± 标准差):']
        for i in range(num_classes):
            lines.append(f'\n[Class {i}]')
...
def main():
    ...
    parser.add_argument('--checkpoint', type=str, default='/home/yuwenjing/DeepLearning_ywj/tta/tent/checkpoints_CT/unet3d_best_CT.pth', help='预训练权重路径，或 "default" 使用默认')
