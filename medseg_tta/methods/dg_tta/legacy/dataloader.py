...
class niiDataset(Dataset):
    ...
    def __init__(self, image_path, img, phase='test'):
        ...
        self.img = img
        self.phase = phase
        self.case_names = self._get_valid_cases()
        if phase == 'train':
    ...
    def _get_valid_cases(self):
        valid_cases = []
        for entry in os.listdir(self.img_path):
            if entry.startswith('.') or not os.path.isdir(join(self.img_path, entry)):
                continue
            case_dir = join(self.img_path, entry)
            img_file = join(case_dir, f'{entry}-{self.img}.nii.gz')
            seg_file = join(case_dir, f'{entry}-seg.nii.gz')
            if os.path.exists(img_file) and os.path.exists(seg_file):
                valid_cases.append(entry)
            else:
                print(f'跳过病例 {entry}：文件缺失（图像：{('存在' if os.path.exists(img_file) else '缺失')}，标签：{('存在' if os.path.exists(seg_file) else '缺失')}）')
        return valid_cases

    def __getitem__(self, index):
        name = self.case_names[index]
        base_dir = join(self.img_path, name)
        if not os.path.isdir(base_dir):
            raise RuntimeError(f'病例目录异常：{base_dir}')
        img_file = join(base_dir, f'{name}-{self.img}.nii.gz')
        seg_file = join(base_dir, f'{name}-seg.nii.gz')
        imga = np.array(nib_load(img_file), dtype='float32')
        mask = np.array(nib_load(seg_file), dtype='float32')
...
if __name__ == '__main__':
    ...
    img = 't2f'
    train_loader, test_loader = get_data_loader(batch_train, batch_test, nw=num_workers, img=img, mode=mode)
    print('数据加载完成')
