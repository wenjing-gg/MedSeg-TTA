...
class MIND3D(torch.nn.Module):
    ...
    def __init__(self, delta=1, sigma=1, randn_weighting=0.05) -> None:
        ...
        six_neighbourhood = torch.tensor([[0, 1, 1], [1, 1, 0], [1, 0, 1], [1, 1, 2], [2, 1, 1], [1, 2, 1]], dtype=torch.float)
    ...
    def forward(self, img):
        device = img.device
        edge_selection = F.conv3d(self.rpad(img), self.mshift1.to(device), dilation=self.delta) - F.conv3d(self.rpad(img), self.mshift2.to(device), dilation=self.delta)
